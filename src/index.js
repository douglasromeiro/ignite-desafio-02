...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.todos.length >= 10 && user.pro === false) {
    return response.status(403).json({ error: "Paga o plano safado!" });
  }

  request.user.todos = user.todos;
...
function checksTodoExists(request, response, next) {
  ...
  const user = users.find((user) => user.username === username);
  const verifyId = regexExp.test(idTodo);
  ...
  if (!user) {
    ...
    return response.status(404).json({ error: "Todo not found in user" });
  }
